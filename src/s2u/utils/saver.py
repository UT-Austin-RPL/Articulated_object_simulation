...
def get_mesh_pose_dict_from_world(world,
    ...
    for uid in world.bodies.keys():
        ...
        for visual in world.p.getVisualShapeData(uid):
            object_name, mesh_path, scale, pose = get_mesh_pose(
                visual, world.p._client)
            mesh_path = mesh_path.decode('utf8')
            if object_name not in mesh_pose_dict.keys():
                mesh_pose_dict[object_name] = []
            if mesh_path == '':  # primitive
                linkIndex = visual[1]
                mesh_path = f'#{uid}_{linkIndex}'
            mesh_pose_dict[object_name].append((mesh_path, scale, pose.as_matrix()))
